...
class B_Aspek extends Library {
    ...
    protected function getAll() {
        $this->app->get($this->pattern, function(Request $request, Response $response) {
            $Query = "CALL sp_mstaspek('getAll', NULL, NULL, NULL, NULL, NULL);";
    ...
    protected function getData() {
        $this->app->get($this->pattern.'/{VALUE_DATA}[/{KOLOM}]', function(Request $request, Response $response, $args) {
            ...
            if (empty($args['KOLOM'])) {
                $Query = "CALL sp_mstaspek('getData', 'idAspek', $value_data, NULL, NULL, NULL);";
            } if (!empty($args['KOLOM'])) {
                $kolom = $args['KOLOM'];
                $Query = "CALL sp_mstaspek('getData', 'namaAspek', NULL, '%$value_data%', NULL, NULL);";
            }
    ...
    protected function post() {
        $this->app->post($this->pattern.'/', function(Request $request, Response $response) {
            ...
            $Query = "CALL sp_mstaspek('create', NULL, NULL, '$dataParsed[namaAspek]', '$dataParsed[createdBy]', NULL);";
    ...
    protected function put() {
        $this->app->put($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstaspek('update', NULL, $dataParsed[idAspek], '$dataParsed[namaAspek]', NULL, '$dataParsed[updatedBy]');";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
    ...
    protected function delete() {
        $this->app->delete($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstaspek('delete', NULL, $dataParsed[idAspek], NULL, NULL, NULL);";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
