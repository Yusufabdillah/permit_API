...
class B_Referensi extends Library {
    ...
    protected function getAll() {
        $this->app->get($this->pattern, function(Request $request, Response $response) {
            $Query = "CALL sp_mstrefpersyaratan('getAll', NULL, NULL, NULL, NULL, NULL, NULL, NULL);";
    ...
    protected function getData() {
        $this->app->get($this->pattern.'/{VALUE_DATA}[/{KOLOM}]', function(Request $request, Response $response, $args) {
            ...
            if (empty($args['KOLOM'])) {
                $Query = "CALL sp_mstrefpersyaratan('getData', 'idReferensi', $value_data, NULL, NULL, NULL, NULL, NULL);";
                $Fetch = $this->db->query($Query)->fetch(PDO::FETCH_OBJ);
            } if (!empty($args['KOLOM'])) {
                $kolom = $args['KOLOM'];
                if ($kolom == 'idJudul') {
                    $Query = "CALL sp_mstrefpersyaratan('getData', 'idJudul', NULL, $value_data, NULL, NULL, NULL, NULL);";
                } else if ($kolom == 'idPersyaratan') {
                    $Query = "CALL sp_mstrefpersyaratan('getData', 'idPersyaratan', NULL, NULL, $value_data, NULL, NULL, NULL);";
                }
                $Fetch = $this->db->query($Query)->fetchAll(PDO::FETCH_OBJ);
            }
            if (!empty($Fetch)) {
    ...
    protected function post() {
        $this->app->post($this->pattern.'/', function(Request $request, Response $response) {
            ...
            $Query = "CALL sp_mstrefpersyaratan('create', NULL, NULL, $dataParsed[idJudul], $dataParsed[idPersyaratan], '$dataParsed[deskripsiReferensi]', '$dataParsed[createdBy]', NULL);";
    ...
    protected function put() {
        $this->app->put($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstrefpersyaratan('update', NULL, $dataParsed[idReferensi], $dataParsed[idJudul], $dataParsed[idPersyaratan], '$dataParsed[deskripsiReferensi]', NULL, '$dataParsed[updatedBy]');";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
    ...
    protected function delete() {
        $this->app->delete($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstrefpersyaratan('delete', NULL, $dataParsed[idReferensi], NULL, NULL, NULL, NULL, NULL);";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
