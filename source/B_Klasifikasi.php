...
class B_Klasifikasi extends Library {
    ...
    protected function getAll() {
        $this->app->get($this->pattern, function(Request $request, Response $response) {
            $Query = "CALL sp_mstklasifikasi('getAll', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);";
    ...
    protected function getData() {
        $this->app->get($this->pattern.'/{VALUE_DATA}[/{KOLOM}]', function(Request $request, Response $response, $args) {
            ...
            if (empty($args['KOLOM'])) {
                $Query = "CALL sp_mstklasifikasi('getData', 'idKlasifikasi', $value_data, NULL, NULL, NULL, NULL, NULL, NULL);";
                $Fetch = $this->db->query($Query)->fetch(PDO::FETCH_OBJ);
            } if (!empty($args['KOLOM'])) {
                $kolom = $args['KOLOM'];
                if ($kolom == 'namaKlasifikasi') {
                    $Query = "CALL sp_mstklasifikasi('getData', 'namaKlasifikasi', NULL, NULL, NULL, '%$value_data%', NULL, NULL, NULL);";
                } if ($kolom == 'idSubk') {
                    $Query = "CALL sp_mstklasifikasi('getData', 'idSubk', NULL, NULL, $value_data, NULL, NULL, NULL, NULL);";
                } if ($kolom == 'idSub_subk') {
                    $Query = "CALL sp_mstklasifikasi('getData', 'idSub_subk', NULL, $value_data, NULL, NULL, NULL, NULL, NULL);";
                }
                $Fetch = $this->db->query($Query)->fetchAll(PDO::FETCH_OBJ);
            }
            if ($Fetch) {
    ...
    protected function post() {
        $this->app->post($this->pattern.'/', function(Request $request, Response $response) {
            ...
            $Query = "CALL sp_mstklasifikasi('create', NULL, NULL, $dataParsed[idSub_subk], NULL, '$dataParsed[namaKlasifikasi]', $dataParsed[kompleksitasKlasifikasi], '$dataParsed[createdBy]', NULL);";
    ...
    protected function put() {
        $this->app->put($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstklasifikasi('update', NULL, $dataParsed[idKlasifikasi], $dataParsed[idSub_subk], NULL, '$dataParsed[namaKlasifikasi]', $dataParsed[kompleksitasKlasifikasi], NULL, '$dataParsed[updatedBy]');";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
    ...
    protected function delete() {
        $this->app->delete($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstklasifikasi('delete', NULL, $dataParsed[idKlasifikasi], NULL, NULL, NULL, NULL, NULL, NULL);";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
