...
class B_Divisi extends Library {
    ...
    protected function getAll() {
        $this->app->get($this->pattern, function(Request $request, Response $response) {
            $Query = "CALL sp_mstdivisi('getAll', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);";
    ...
    protected function getData() {
        $this->app->get($this->pattern.'/{VALUE_DATA}[/{KOLOM}]', function(Request $request, Response $response, $args) {
            ...
            if (empty($args['KOLOM'])) {
                $Query = "CALL sp_mstdivisi('getData', 'idDivisi', $value_data, NULL, NULL, NULL, NULL, NULL, NULL);";
            } if (!empty($args['KOLOM'])) {
                $kolom = $args['KOLOM'];
                $Query = "CALL sp_mstdivisi('getData', 'namaDivisi', NULL, NULL, '%$value_data%', NULL, NULL, NULL, NULL);";
            }
    ...
    protected function post() {
        $this->app->post($this->pattern.'/', function(Request $request, Response $response) {
            ...
            $Query = "CALL sp_mstdivisi('create', NULL, NULL, $dataParsed[idPerusahaan], '$dataParsed[namaDivisi]', '$dataParsed[singkatanDivisi]', $dataParsed[statusDivisi], '$dataParsed[createdBy]', NULL);";
    ...
    protected function put() {
        $this->app->put($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstdivisi('update', NULL, $dataParsed[idDivisi], $dataParsed[idPerusahaan], '$dataParsed[namaDivisi]', '$dataParsed[singkatanDivisi]', $dataParsed[statusDivisi] , NULL, '$dataParsed[updatedBy]');";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
    ...
    protected function delete() {
        $this->app->delete($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstdivisi('delete', NULL, $dataParsed[idDivisi], NULL, NULL, NULL, NULL, NULL, NULL);";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
