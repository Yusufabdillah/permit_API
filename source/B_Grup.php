...
class B_Grup extends Library {
    ...
    protected function getAll() {
        $this->app->get($this->pattern, function(Request $request, Response $response) {
            $Query = "CALL sp_mstgrupuser('getAll', NULL, NULL, NULL, NULL, NULL, NULL, NULL);";
    ...
    protected function getData() {
        $this->app->get($this->pattern.'/{VALUE_DATA}[/{KOLOM}]', function(Request $request, Response $response, $args) {
            ...
            if (empty($args['KOLOM'])) {
                $Query = "CALL sp_mstgrupuser('getData', 'idGrup', $value_data, NULL, NULL, NULL, NULL, NULL);";
            } if (!empty($args['KOLOM'])) {
                $kolom = $args['KOLOM'];
                $Query = "CALL sp_mstgrupuser('getData', 'namaGrup', NULL, NULL, NULL, '%$value_data%', NULL, NULL);";
            }
    ...
    protected function post() {
        $this->app->post($this->pattern.'/', function(Request $request, Response $response) {
            ...
            $Query = "CALL sp_mstgrupuser('create', NULL, NULL, $dataParsed[idAkses], $dataParsed[idPerusahaan], '$dataParsed[namaGrup]', '$dataParsed[createdBy]', NULL);";
            $Exec = $this->db->prepare($Query)->execute();
            $stmt = $this->db->query("SELECT LAST_INSERT_ID() as 'idGrup'")->fetch(PDO::FETCH_OBJ);
            $idGrup = $stmt->idGrup;
            if ($Exec) {
                return $response->withJson(["status" => "success" , 'idGrup' => $idGrup], 200);
            } else {
                return $response->withJson(["status" => "failed"], 200);
            }
        });
    ...
    protected function put() {
        $this->app->put($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstgrupuser('update', NULL, $dataParsed[idGrup], $dataParsed[idAkses], $dataParsed[idPerusahaan], '$dataParsed[namaGrup]', NULL, '$dataParsed[updatedBy]');";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
    ...
    protected function delete() {
        $this->app->delete($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstgrupuser('delete', NULL, $dataParsed[idGrup], NULL, NULL, NULL, NULL, NULL);";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
