...
class F_Estafet extends Library {
    ...
    {
        $deklarasi['view'] = 'vw_mstestafet';
        $deklarasi['tabel'] = 'tbl_mstestafet';
        $deklarasi['pk'] = 'idEstafet';
    }
    ...
    protected function getAll() {
     ...
     private function getByPengajuan() {
        $this->app->get($this->pattern, function(Request $request, Response $response) {
    ...
    private function getDataHistory() {
        $this->app->get($this->pattern, function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Fetch = $this->qb->table($this->view)
                ->where($this->pk, $dataParsed[$this->pk])
                ->first();
            $dataHistory = json_decode($Fetch->historyEstafet);
            if (!empty($Fetch)) {
                return $response->withJson(['status' => 'success', 'data' => $dataHistory], 200);
            } else {
                return $response->withJson(["status" => "failed"], 500);
            }
        })->add(parent::middleware());
