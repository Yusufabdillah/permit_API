...
class B_Perusahaan extends Library {
    ...
    protected function getAll() {
        $this->app->get($this->pattern, function(Request $request, Response $response) {
            $Query = "CALL sp_mstperusahaan('getAll', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);";
    ...
    protected function getData() {
        $this->app->get($this->pattern.'/{VALUE_DATA}[/{KOLOM}]', function(Request $request, Response $response, $args) {
            ...
            if (empty($args['KOLOM'])) {
                $Query = "CALL sp_mstperusahaan('getData', 'idPerusahaan', $value_data, NULL, NULL, NULL, NULL, NULL, NULL);";
                $Fetch = $this->db->query($Query)->fetch(PDO::FETCH_OBJ);
            } if (!empty($args['KOLOM'])) {
                $kolom = $args['KOLOM'];
                if ($kolom == 'idKecamatan') {
                    $Query = "CALL sp_mstperusahaan('getData', 'idKecamatan', NULL, $value_data, NULL, NULL, NULL, NULL, NULL);";
                } if ($kolom == 'namaPerusahaan') {
                    $Query = "CALL sp_mstperusahaan('getData', 'namaPerusahaan', NULL, NULL, '%$value_data%', NULL, NULL, NULL, NULL);";
                }
                $Fetch = $this->db->query($Query)->fetchAll(PDO::FETCH_OBJ);
            }
            if ($Fetch) {
    ...
    protected function post() {
        $this->app->post($this->pattern.'/', function(Request $request, Response $response) {
            ...
            $Query = "CALL sp_mstperusahaan('create', NULL, NULL, $dataParsed[idKecamatan], '$dataParsed[namaPerusahaan]', '$dataParsed[singkatanPerusahaan]', '$dataParsed[lokasiPerusahaan]', '$dataParsed[createdBy]', NULL);";
    ...
    protected function put() {
        $this->app->put($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstperusahaan('update', NULL, $dataParsed[idPerusahaan], $dataParsed[idKecamatan], '$dataParsed[namaPerusahaan]', '$dataParsed[singkatanPerusahaan]', '$dataParsed[lokasiPerusahaan]' , NULL, '$dataParsed[updatedBy]');";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
    ...
    protected function delete() {
        $this->app->delete($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstperusahaan('delete', NULL, $dataParsed[idPerusahaan], $dataParsed[idKecamatan], NULL, NULL, NULL, NULL, NULL);";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
